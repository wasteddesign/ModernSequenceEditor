...
namespace WDE.ModernSequenceEditor
{
    class SelectionLayer : Canvas, INotifyPropertyChanged
    {
        ...
        public Rect Rect
        {
			get
			{
				int start = SequenceEditor.ViewSettings.TimeSignatureList.Snap((int)Math.Min(point.Y, anchor.Y), int.MaxValue);
				int end = SequenceEditor.ViewSettings.TimeSignatureList.Snap((int)Math.Max(point.Y, anchor.Y), int.MaxValue);
