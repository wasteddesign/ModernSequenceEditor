...
namespace WDE.ModernSequenceEditor
{
    ...
    public partial class SequenceEditor : UserControl, INotifyPropertyChanged
    {
        ...
        //Dictionary<IPattern, int> patternsToNotify = new Dictionary<IPattern, int>();
            ...
            switch (e.PropertyName)
            {
                ...
                case "Playing":
                    if (Buzz.Playing && Settings.AutoUpdateEventHints)
                    {
                        ...
                        {
							foreach(PatternElement pe in tc.eventCanvas.Children)
                            {
								Task.Factory.StartNew(() =>
								{
									Task.Delay(50);
									pe.UpdatePatternEvents();
                    ...
                    // Old seq editor updates the midi focus. We could force it to be what is defined in MSE:

                    //if (Settings.ForceMIDIFocus)
                    //{
        ...
        public static WDE.ModernSequenceEditor.SequenceEditorSettings Settings = new SequenceEditorSettings();
        public WDE.ModernSequenceEditor.SequenceEditorSettings BindableSettings { get { return Settings; } }
        ...
        public SequenceEditor(IBuzz buzz, ResourceDictionary rd, bool integraetedToBuzz = false)
        {
            ...
            SettingsWindow.AddSettings("Modern Sequence Editor", Settings);
            ...
            DispatcherTimerHold.Tick += (sender, e) =>
            {
                ...
                int snapTime = viewSettings.TimeSignatureList.Snap(time, int.MaxValue);

                while (snapTime >= 0)
                {
                    snapTime = viewSettings.TimeSignatureList.Snap(snapTime, int.MaxValue);
            ...
            SetPatternColorCommand = new SimpleCommand
            {
                ...
                ExecuteDelegate = index =>
                {
                    ...
                    viewSettings.PatternAssociationsChangedEvent();
                }
            };
            ...
            CreatePatternsCommand = new SimpleCommand
            {
                ...
                ExecuteDelegate = num =>
                {
                    ...
                    using (new ActionGroup(viewSettings.EditContext.ActionStack))
                    {
                        ...
                        // 1
                        if (!insert)
                            Do(new TSLInsertOrDeleteAction(viewSettings.TimeSignatureList, numPatterns * Settings.PatternLength, CursorSpan, false)); // Delete
                        Do(new TSLInsertOrDeleteAction(viewSettings.TimeSignatureList, numPatterns * Settings.PatternLength, CursorSpan, true)); // Insert
        ...
        public void SetViewSettings(TimeSignatureList tsl, double zoom, Tuple<string, string, int>[] patternAsso, Tuple<int, string, string>[] vus)
        {
            ...
            viewSettings.TimeSignatureList = new TimeSignatureList(tsl);
            viewSettings.TimeSignatureList.Changed += TimeSignatureListChanged;
        ...
        private void Button_Click_Refresh_Notes(object sender, RoutedEventArgs e)
        {
            foreach (TrackControl tc in trackStack.Children)
            {
                tc.UpdateNoteHints();
