...
namespace WDE.ModernSequenceEditor
{
    public class CursorElement : FrameworkElement
	{
		...
		public void Update()
		{
			...
			Height = SequenceEditor.ViewSettings.TimeSignatureList.GetBarLengthAt(Time) * SequenceEditor.ViewSettings.TickHeight + 1;
		...
		public void Move(int dx, int dy, int maxx)
		{
			if (dy != 0)
			{
				long t = Time + (long)dy * SequenceEditor.ViewSettings.TimeSignatureList.GetBarLengthAt(Time);
				Time = SequenceEditor.ViewSettings.TimeSignatureList.Snap(t, maxx);
			}
